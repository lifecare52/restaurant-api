  ...
  try {
    ...
    if (measurements.length === 0) {
      ...
      const defaultMeasurements = [
        ...
        {
          name: 'Custom Box',
          measurementType: 'CUSTOM' as const,
          unit: 'box',
          baseUnit: 'box',
          conversionFactor: 1,
          isDecimalAllowed: false
    ...
    for (const vName of varNames) {
      ...
      if (existing) {
        ...
        const created = await VariationEntity.create({
          brandId,
          outletId,
          name: vName,
          department: VariationDepartment.CUSTOM
    ...
    for (let i = 0; i < measurements.length; i++) {
      ...
      for (let j = 0; j < itemDefs.length; j++) {
        ...
        const shortCode = `SMEAS${i}${j}${Math.floor(Math.random() * 1000)
          .toString()
          .padStart(3, '0')}`;
        ...
        for (let v = 0; v < variations.length; v++) {
          ...
          const existingVariant = await MenuItemVariantEntity.findOne({
            brandId,
            outletId,
