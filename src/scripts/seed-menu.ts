...
async function seedData() {
    try {
        ...
        for (const addon of addonsToCreate) {
            ...
            const existing = await AddonEntity.findOne({ brandId, outletId, name: { $regex: new RegExp(`^${addon.name}$`, 'i') } });
            if (existing) {
        ...
        for (const v of variationsToCreate) {
            const existing = await VariationEntity.findOne({ brandId, outletId, department: v.department, name: { $regex: new RegExp(`^${v.name}$`, 'i') } });
            if (existing) {
        ...
        const itemNames = [
            ...
            { name: 'Chocolate Brownie with Ice Cream', categoryIndex: 3, dietary: Dietary.EGG, basePrice: 220 },
        ...
        for (let i = 0; i < itemNames.length; i++) {
            ...
            if (itemDef.categoryIndex === 1 || itemDef.categoryIndex === 4) { // Main course or beverages
