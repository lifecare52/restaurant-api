...
const tagNameSchema = Joi.string()
  .trim()
  .min(2)
  .max(100)
...
const validateDiscountRules = (value: Record<string, unknown>, helpers: Joi.CustomHelpers) => {
  ...
  if (
    ...
    return helpers.message({
      custom: 'discountValue must be between 0.01 and 100 when discount type is PERCENTAGE',
    });
  }

  if (discountType === CUSTOMER_TAG_DISCOUNT_TYPE.FLAT && discountValue <= 0) {
    return helpers.message({
      custom: 'discountValue must be greater than 0 when discount type is FLAT',
    });
  }
